...
export async function handleVerification({
	...
	if (!newEmail) {
		submission.error[''] = [
			'You must submit the code on the same device you requested the email change.',
		]
		return json(
			{
				status: 'error',
	...
	const { email: oldEmail } = await prisma.user.findUniqueOrThrow({
	...
	void sendEmail({
		to: oldEmail,
		subject: 'Notice: Epic Note email has been changed',
		react: <EmailChangeNoticeEmail userId={submission.value.target} />,
	})
	const toast: OptionalToast = {
		title: 'Email Change',
		description: `You've successfully changed your email to ${newEmail}`,
		type: 'success',
	}

	throw await redirectWithToast('/settings/profile', toast, {
		headers: {
